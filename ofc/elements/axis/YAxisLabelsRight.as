		...
		public function YAxisLabelsRight( parent:YAxisRight, json:Object ) {
			...
			if( json.y_axis_right )
			{
				...
				if( json.y_axis_right.labels is Array )
				{
					...
					var i:Number = parent.style.min;
					var steps:Number = parent.style.min > parent.style.max ? -1 : 1;
					...
					i = isNaN(json.y_axis_right.max) ? values.length - 1 : json.y_axis_right.max;
					parent.set_y_max( i );
					ok = true;
				...
				else if ( json.y_axis_right.labels is Object ) 
				{
					...
					if ( json.y_axis_right.labels.text is String ) this.lblText = json.y_axis_right.labels.text;
