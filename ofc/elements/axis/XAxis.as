	...
	public class XAxis extends Sprite {
		...
		public function three_d_axis( sc:ScreenCoords ):void
		{
			...
			if (this.user_ticks) 
			{
				...
				{
					var xVal:Number = sc.get_x_from_val(lbl.x);
					// Draw the tick down from the front
					this.graphics.beginFill(this.colour, 1);
					this.graphics.drawRect( xVal-offset, sc.bottom+offset+h, w, this.tick_height );
					this.graphics.endFill();
				}
			}
			else
			{
				var rev:Boolean = (this.style.min >= this.style.max); // min-max reversed?
				for( var i:Number=0; rev ? i >= this.style.max : i <= this.style.max; i+=this.steps )
				{
					var pos:Number = sc.get_x_tick_pos(i);
					
					// Draw the tick down from the front
					this.graphics.beginFill(this.colour, 1);
					this.graphics.drawRect( pos-offset, sc.bottom+offset+h, w, this.tick_height );
					this.graphics.endFill();
				}
			}
