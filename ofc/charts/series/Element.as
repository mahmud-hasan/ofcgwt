package charts.series {
	...
	public class Element extends Sprite implements has_tooltip {
		...
		protected var origAlpha:Number;
		...
		public function mouseOut(event:Event):void {
			...
			if (isNaN(this.origAlpha)) {
				Tweener.addTween(this, { alpha:1, time:0.4, transition:Equations.easeOutElastic } );
			} else {
				Tweener.addTween(this, { alpha:this.origAlpha, time:0.4, transition:Equations.easeOutElastic } );
			}
		...
		private function mouseUp(event:Event):void {
			...
			if ( this.link.substring(0, 6) == 'trace:' ) {
				...
				trace( this.link, this.index, this.on_click_text );
			}
			else if ((this.link.indexOf(":") > 0) && (this.link.indexOf(":") < 7))
				this.browse_url( this.link );
			else
				ExternalInterface.call( this.link, this.index, this.on_click_text );
		}
