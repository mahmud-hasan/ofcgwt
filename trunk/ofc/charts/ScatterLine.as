package charts {
	...
	public class ScatterLine extends ScatterBase
	{
		...
		public function solid_line( sc:ScreenCoordsBase ): void {
			...
			var last_x:Number = 0;
			var last_y:Number = 0;
			var right_axis:Boolean = (this.style.axis == "right"); 
			...
			if (areaStyle != null)
			{
				...
				if (!isNaN(areaBaseY)) 
				{
					...
					areaBaseY = sc.get_y_from_val(areaBaseY, right_axis);  // TODO: Allow for right Y-Axis??
				}
			}
			...
			for ( var i:Number = 0; i < this.numChildren; i++ ) {
				...
				if( tmp is Element )
				{
					...
					e.resize( sc );
					if (e._y == Number.NEGATIVE_INFINITY)
					{
